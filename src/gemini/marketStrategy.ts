...
export async function generateMarketStrategy({
  ...
  const systemInstruction = [
    ...
    'Separate buyer-demand signals from pricing comps. Never imply that the system contacted, messaged, or posted to a buyer; research is discovery only until the owner approves.',
  ...
  const prompt = [
    ...
    '## Buyer Demand',
    '- Summarize public buyer-demand channels found by browser-use. Name Craigslist/Reddit/Apify channels if present, note weak or skipped sources, and state the safest outreach order without claiming any messages were sent.',
    '## Pricing Strategy',
    '- Recommended list price, expected close price, floor guardrail, first 48-hour plan, negotiation script.',
    '## Marketplace Listing',
    '- Draft only: title, short description, included items, condition note, safety/payment note, and search keywords. Make clear the owner reviews before posting.',
...
export function buildResearchTask(userText: string, profile: ItemIntakeProfile) {
  ...
  return [
    ...
    'First look for public buyer demand from Craigslist wanted posts and Reddit WTB searches.',
    'Then compare pricing comps from Craigslist for-sale listings, eBay sold listings, Swappa, Back Market, and any configured Apify actor sources.',
    'Do not log in, post, message buyers, scrape private contact info, enter checkout/payment flows, or bypass captchas.',
